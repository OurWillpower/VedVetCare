...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Start Slider FIRST (Priority)
    initHeroSlider(); 
    
    // 2. Load Products
    loadProducts();
    
    // 3. Animations (Wrapped in try-catch so they don't break the site if GSAP fails)
    try {
...
// --- HERO SLIDER LOGIC ---
let currentSlide = 1;
const totalSlides = 7; // 6 Animals + 1 Collage
let slideInterval;

function initHeroSlider() {
    console.log("Slider Initialized"); // Debug check
...
function showSlide(slideIndex) {
    // Reset interval if user clicks manually to avoid double-jumps
